...
iteration_covariates_base = list(
  c("AgeAtPreg", "BirthYear"),
  c("AgeAtPreg", "BirthYear", "TownsendDeprivationIndex", "University"),
  c("AgeAtPreg", "BirthYear", "TownsendDeprivationIndex", "University", "parity", "rpl"))
...
# filter
MODEL_combined_filtered = MODEL_combined %>%
  ...
  filter(five_plus == 1) %>% # keep only rows with frequencies >= 5 
  group_by(iteration) %>%
  mutate(
...
# main analysis
output_to_table = MODEL_combined_filtered %>%
  filter(iteration == 3) %>%
