...
demo = fread("raw_UKB/demo.csv", na.strings=c("", NA))
...
pregnancies_2 = pregnancies_1 %>%
  ...
  filter(Date >= as.Date("1950-01-01", format = "%Y-%m-%d"), # remove rows with dates before 1950
         ...
         eid %in% demo$`Participant ID`[demo$Sex == "Female"], # remove males
...
pregnancies_8 = pregnancies_7 %>%
  left_join(demo %>% select(`Participant ID`, `Year of birth`), by = c("eid" = "Participant ID")) %>%
