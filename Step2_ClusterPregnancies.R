...
# ===============================================================================
# CLUSTER ICD CODES TO TRANSFORM THE DATASET FROM PERSON-LEVEL TO PREGNANCY-LEVEL 
# ===============================================================================
...
# load data
Hospital = fread("Data/processed/Step1A_ExtractPregnancyCodes_Hospital.csv", na.strings=c("", NA))
PrimaryCare = fread("Data/processed/Step1B_ExtractPregnancyCodes_PrimaryCare.csv", na.strings=c("", NA))
withdrawn_from_UKB = fread("Data/processed/withdrawn_from_UKB.csv", na.strings=c("", NA)) #**
UKB_vars = fread("Data/raw_UKB/Pathways_All_Vars.csv", na.strings=c("", NA))
GP_ClinicalEventRecord = fread("Data/raw_UKB/gp_clinical.csv", na.strings=c("", NA)) %>% pull(`Participant ID`) %>% unique()
...
# is more than [gap_days] days from the anchor date, that becomes the new anchor date and starts a new pregnancy.
...
pregnancies_2 = pregnancies_1 %>%
  ...
  filter(Date >= as.Date("1950-01-01", format = "%Y-%m-%d"), # remove rows with dates before 1950
         !eid %in% withdrawn_from_UKB$eid & # remove the withdrawn individuals
         !eid %in% UKB_vars$eid[UKB_vars$Sex=="Male"], # remove males
         eid %in% GP_ClinicalEventRecord) # remove individuals without primary care linkage


# clean the environment
rm(Hospital, PrimaryCare, preg_categories, remove_EIDs, UKB_vars, GP_ClinicalEventRecord)
...
# remove the deliveries and abortive outcome too close to one another to be biologically possible
pregnancies_flag_short = pregnancies_3 %>% 
  distinct(eid, deliv_ID_mod, max_date_deliv_mod, abort_ID_mod, max_date_abort_mod) %>%
  mutate(date_ = coalesce(max_date_deliv_mod, max_date_abort_mod)) %>% # (temporary date column)
  filter(!is.na(date_)) %>%
  group_by(eid) %>%
  arrange(eid, date_) %>%
...
df_Undefined = pregnancies_4 %>%
  filter(is.na(pregnancy)) %>%   
  arrange(eid, Date) %>%                       
  group_by(eid) %>%
  group_modify(~ group_by_anchor(.x, .y, date_col = "Date", gap_days = gap_days)) %>%
  ungroup() %>%                     
  mutate(undef_ID = paste("undef", grouper, sep = "_")) %>% 
  group_by(eid, undef_ID) %>%
  mutate(max_date_undef = max(Date)) %>%
  ungroup() %>%
  select(-grouper)


# merge undefined subset back with the pregnancy cohort
pregnancies_5 = pregnancies_4 %>%
  full_join(df_Undefined) %>%
  mutate(pregnancy = coalesce(pregnancy, undef_ID),
         max_date_preg = as.Date(max_date_preg),
         max_date_preg = coalesce(max_date_preg, max_date_undef)) %>%
  select(-c(undef_ID, max_date_undef)) %>%
...
# collapse certain undefined pregnancies with deliveries or abortive outcomes
pregnancies_collapsed = pregnancies_5 %>%
  ...
  mutate(
    ...
    collapse_deliv = (is_deliv | is_undef) & 
      (lag_is_deliv | lag_is_undef) &
      !(is_undef & lag_is_undef) &
      as.numeric(max_date_preg - lag(min_date_preg)) < deliv_window,
    
    collapse_abort = (is_abort | is_undef) & 
      (lag_is_abort | lag_is_undef) &
      !(is_undef & lag_is_undef) &
      as.numeric(max_date_preg - lag(min_date_preg)) < abort_window,
    
    # collapsing logic
    pregnancy_mod = case_when(
      is_undef & lead_is_deliv & lead(collapse_deliv) ~ lead(pregnancy),
      is_undef & lag_is_deliv & collapse_deliv        ~ lag(pregnancy),
      is_deliv & lag_is_deliv & collapse_deliv        ~ lag(pregnancy),
      
      is_undef & lead_is_abort & lead(collapse_abort) ~ lead(pregnancy),
      is_undef & lag_is_abort & collapse_abort        ~ lag(pregnancy),
...
pregnancies_7 = pregnancies_6 %>%
  mutate(pregnancy_type = sub("\\d+", "", pregnancy)) %>%  # extract the pregnancy outcomes ("deliv", "abort", "undef")
  group_by(eid, pregnancy_type) %>%
  mutate(unique_count = dense_rank(max_date_preg),
         pregnancy = paste(pregnancy_type, unique_count, sep = "")) %>%  # modify pregnancy labels to reflect modified numbers
  ungroup() %>%
  select(-c(pregnancy_type, unique_count, collapse_deliv, collapse_abort, pregnancy_mod, Delivery, Abortive, overlap)) %>%
  group_by(eid, pregnancy) %>%
  mutate(max_date_preg = max(Date),
         min_date_preg = min(Date)) %>%
  ungroup()
...
# export
write.csv(pregnancies_7,"Data/processed/Step2_ClusterPregnancies.csv",row.names=F)


# offload script-specific packages
detach("package:zoo", unload = TRUE, character.only = TRUE)
