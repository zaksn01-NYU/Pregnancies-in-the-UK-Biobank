...
# ==========================================================================
# DEFINE OBSTETRIC OUTCOMES AND AND TRANSFORM THE DATASET TO PREGNANCY-LEVEL
# ==========================================================================
...
# load data
pregnancies = fread("Data/processed/Step5_AddAdditionalVars.csv", na.strings=c("", NA))
...
pregnancies_1 = pregnancies %>%
  select(eid, eid_pregnancy, max_date_preg, AgeAtPreg, BirthYear, TownsendDeprivationIndex, University, White_eth, 
  ...
  mutate(across(-c(max_date_preg,AgeAtPreg,BirthYear,TownsendDeprivationIndex,matches("PRS_|^PC")), as.factor)) %>%
  distinct()
  

# add a variable for parity
...
# export
write.csv(pregnancies_1,"Data/processed/Step6_DefinePregnancyOutcomes.csv", row.names=F)
